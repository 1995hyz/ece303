...
class Sender(object):
    ...
    def __init__(self, inbound_port=50006, outbound_port=50005, timeout=10, debug_level=loggidef checkSum(self,data):        #this function converts data into a bytearray, and does a XOR sum on each elements of the byte-array. Return the invert of the XOR sum
        byteData=bytearray(data)
        xorSum=bytes(0)
        for i in xrange(len(byteData)):
            xorSum^=byteData[i]
...
if __name__ == "__main__":
    ...
    sndr = BogoSender()
    sndr.send(BogoSender.TEST_DATA)
>>>>>>> 3a767b325412df315ec65684b02247ba1444487d
